...
GROESTLCOIN_CONFIG_ALL="-DBUILD_BENCH=OFF -DBUILD_FUZZ_BINARY=OFF"
if [ -z "$NO_DEPENDS" ]; then
  GROESTLCOIN_CONFIG_ALL="${GROESTLCOIN_CONFIG_ALL} -DCMAKE_TOOLCHAIN_FILE=$DEPENDS_DIR/$HOST/toolchain.cmake"
fi
if [ -z "$NO_WERROR" ]; then
  GROESTLCOIN_CONFIG_ALL="${GROESTLCOIN_CONFIG_ALL} -DWERROR=OFF"
fi
...
GROESTLCOIN_CONFIG_ALL="$GROESTLCOIN_CONFIG_ALL -DCMAKE_INSTALL_PREFIX=$BASE_OUTDIR"

if [[ "${RUN_TIDY}" == "true" ]]; then
  GROESTLCOIN_CONFIG_ALL="$GROESTLCOIN_CONFIG_ALL -DCMAKE_EXPORT_COMPILE_COMMANDS=ON"
fi

bash -c "cmake -S $BASE_ROOT_DIR $GROESTLCOIN_CONFIG_ALL $GROESTLCOIN_CONFIG || ( (cat $(cmake -P "${BASE_ROOT_DIR}/ci/test/GetCMakeLogFiles.cmake")) && false)"
