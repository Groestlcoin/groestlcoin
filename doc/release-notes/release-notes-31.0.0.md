Groestlcoin Core version v31.0 is now available from:
...
(on Windows) or just copy over `/Applications/Groestlcoin-Qt` (on macOS) or
...
migrated. Old wallet versions of Groestlcoin Core are generally supported.

Compatibility
==============

Groestlcoin Core is supported and tested on the following operating systems or
newer: Linux Kernel 3.17, macOS 14, and Windows 10 (version 1903). Groestlcoin Core
should also work on most other Unix-like systems but is not as frequently tested
on them. It is not recommended to use Groestlcoin Core on unsupported systems.
...
`-dbcache=450`. See
[reduce-memory.md](https://github.com/groestlcoin/groestlcoin/blob/master/doc/reduce-memory.md)
for further guidance on low-memory systems.
...
[doc/policy/mempool-terminology.md](https://github.com/groestlcoin/groestlcoin/blob/master/doc/policy/mempool-terminology.md)
and
[doc/policy/mempool-replacements.md](https://github.com/groestlcoin/groestlcoin/blob/master/doc/policy/mempool-replacements.md).
...
without fetching the full transaction data at verbosity 2+.
...
being deprecated in Groestlcoin Core 30.0. They used to allow the user to set a
...
deprecated and has no effect anymore since v30.0.
...
function name instead of the entire function signature.
...
transaction was not found in the mempool.

GUI changes
-----------

- The GUI has been updated to Qt 6.8.

- The `createwallet`, `createwalletdescriptor` and `migratewallet` commands are
filtered from the console history to improve security and privacy.
...
The Groestlcoin Core fee estimator minimum fee rate bucket was updated from **1
gro/vB** to **0.1 gro/vB**, which matches the node’s default `minrelaytxfee`. This
means that for a given confirmation target, if a sub-1 gro/vB fee rate bucket is
...
schema and regenerate bindings to continue working.
...
a flood of templates during startup, but is not guaranteed.
