...
```bash
./build/bin/groestlcoin node -signet -daemon
```
...
for ((n=1;n<=3;n++))
do
 xpubs["xpub_${n}"]=$(./build/bin/groestlcoin rpc -signet -rpcwallet="participant_${n}" derivehdkey "m/44h/17h/0h" | jq -r '.origin + .xpub')
done
```
...
desc_sum=$(./build/bin/groestlcoin rpc -signet getdescriptorinfo $desc | jq -r '.checksum')
...
```bash
./build/bin/groestlcoin rpc -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/groestlcoin rpc -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/groestlcoin rpc -signet -rpcwallet="multisig_wallet_01" listdescriptors
```
...
```bash
combined_psbt=$(./build/bin/groestlcoin rpc -signet combinepsbt txs="[$psbt_1, $psbt_2]")
```
