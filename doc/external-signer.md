...
```sh
groestlcoin rpc createwallet wallet_name="hww2" disable_private_keys=true descriptors=true external_signer=true
```

Creation of the external wallet can be confirmed with `getwalletinfo`, which will report `"external_signer": true`. These commands can also be executed using `groestlcoin-qt` Debug Console instead of using `groestlcoin rpc` or `groestlcoin-cli`.
...
```sh
groestlcoin-cli -rpcwallet=<walletname> getnewaddress
groestlcoin-cli -rpcwallet=<walletname> walletdisplayaddress <address>
```
...
```sh
groestlcoin-cli -rpcwallet=<walletname> sendtoaddress <address> <amount>
```

This constructs a PSBT and prompts your external signer to sign (will fail if it's not connected). If successful, Groestlcoin Core finalizes and broadcasts the transaction.
...
```
{
  "receive": [
    ...
    "tr([00000000/86h/17h/0h]xpub6C.../0/*)#4d8tq2ns"
  ],
  "internal": [
    ...
    "tr([00000000/86h/17h/0h]xpub6C.../1/*)#d63h6jpt"
  ]
}
```
...
For external-signer wallets, spending uses `send` or `sendall`, and fee-bumping uses `bumpfee`. Groestlcoin Core builds a PSBT, adds key origin information, checks whether any input key origin fingerprint matches the signer, calls `<cmd> --stdin --fingerprint 00000000 --chain <name>`, and sends `signtx <psbt>` over stdin. If signatures are sufficient, it finalizes the transaction and, for broadcasting RPCs, broadcasts it. If signing cannot complete, the call fails with an error. For manual fee-bumping, use `psbtbumpfee` to obtain a PSBT for signing.
