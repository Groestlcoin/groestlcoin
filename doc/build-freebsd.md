...
Groestlcoin Core is supported on all [supported FreeBSD releases](https://www.freebsd.org/security/#sup).

This guide describes how to build groestlcoind, command-line utilities, and GUI on the most recent production release.
...
Groestlcoin Core can provide notifications via ZeroMQ. To compile ZMQ support, install the following dependency and pass `-DWITH_ZMQ=ON` when configuring.
```bash
