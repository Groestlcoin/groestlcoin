...
Groestlcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `groestlcoin-cli` is used.
...
Later, when *V* GRS has been received on *Amulti*, and Bob and Carol want to
